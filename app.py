        ...
        if has_visuals:
            ...
            **[참고: 이미지 정밀 분석 결과]**
            ...
            3. **시각적 요소의 정교화 (VECTOR STYLE RECONSTRUCTION):**
               - **대상 범위 (SCOPE):** 이미지 전체를 그리지 마십시오. 문제 번호, 지문(텍스트), 선지(1~5)는 모두 무시하고, **오직 '중앙 삽화(Diagram/Illustration)' 부분만** 추출하여 그리십시오.
            ...
            5. **일관성 유지 (CRITICAL CONSISTENCY):**
               - **기호 일치:** 재구성한 SVG 이미지 내의 기호(A, B, ㉠, ㉡ 등)와 생성한 문제 텍스트(시나리오, 보기, 선지) 내의 지칭 용어가 **100% 일치**해야 합니다.
               - **예시:** 이미지에 'A'라고 적었으면, 문제에서도 반드시 "물체 A는..."라고 서술해야 하며, "물체 (가)는..."이라고 다르게 칭하면 **절대 안 됩니다.**
